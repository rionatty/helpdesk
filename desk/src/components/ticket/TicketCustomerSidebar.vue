  ...
  <div class="flex w-full md:w-[382px] flex-col md:border-s hd-chrome-bg">
    ...
    <div class="flex flex-col gap-4 px-5 py-4 border-b">
      <div class="flex items-center gap-3">
        <Avatar size="2xl" :label="ticket.data.assignee_name || '…'" />
        <div class="flex-1 min-w-0">
          <div class="truncate text-lg font-semibold text-ink-gray-9">
            {{ ticket.data.assignee_name || __("Awaiting assignment") }}
          </div>
          <div class="text-xs text-ink-gray-6 font-medium truncate">
            {{
              ticket.data.assignee_name
                ? __("Handling your ticket")
                : __("An agent will pick this up shortly")
            }}
          </div>
        </div>
        <div v-if="!ticket.data.feedback_rating">
          <Tooltip :text="__('Write a reply')">
    ...
    <div class="px-5 py-4 border-b flex flex-col gap-3">
      ...
      <!-- Support hours (from HD Settings; hidden when not configured) -->
      <div
        v-if="configStore.supportHours"
        class="flex items-center gap-3 text-base"
      >
        <LucideClock class="size-4 text-ink-gray-5 shrink-0" />
        <span class="w-[96px] text-sm font-medium text-ink-gray-7">
          {{ __("Support hours") }}
        </span>
        <span class="text-sm text-ink-gray-8 flex-1">
          {{ configStore.supportHours }}
        </span>
      </div>
      <!-- Subscribe toggle -->
    ...
    <!-- Progress / subtasks (read-only for customers; strip hides when the
         card renders nothing) -->
    <div class="px-5 py-4 border-b [&:not(:has(*))]:hidden">
...
const ticketAdditionalInfo = computed(() => {
  ...
  // Empty rows are noise on the portal — show a field only when it has a value.
